...
#------------------------------------------------------------
    ...
    def test_send_and_get_masterthesis(self):
        with app.app_context():
            methods.send_masterthesis("user4", "Author4", "Title4", "School4", "2022", "", "", "", "")
            methods.send_masterthesis("user5", "Author5", "Title5", "School5", "2023", "", "", "", "")

            masterthesis = methods.get_masterthesis()
            self.assertEqual(len(masterthesis), 2)
    
    def test_send_and_get_masterthesis_all(self):
        with app.app_context():
            methods.send_masterthesis("user8", "Author8", "Title8", "School8", "2018", "type", "address8", "feb", "note8")
            masterthesis = methods.get_masterthesis()
            self.assertEqual(len(masterthesis), 1)
    
    def test_and_send_masterthesis_too_long_user(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis(self.too_long, "Author4", "Title4", "School4", "2022", "", "", "", "")
    
    def test_and_send_masterthesis_too_long_author(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", self.too_long, "Title4", "School4", "2022", "", "", "", "")
    
    def test_and_send_masterthesis_too_long_title(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", self.too_long, "School4", "2022", "", "", "", "")
    
    def test_and_send_masterthesis_too_long_school(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", self.too_long, "2022", "", "", "", "")
    
    def test_and_send_masterthesis_too_long_type(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "2022", self.too_long, "", "", "")

    def test_and_send_masterthesis_too_long_address(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "2022", "", self.too_long, "", "")

    def test_and_send_masterthesis_too_long_note(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "2022", "", "", "", self.too_long)

    def test_and_send_masterthesis_too_long_month(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "2022", "", "", "väärä", "")
    
    def test_and_send_masterthesis_year_invalid_string(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "väärä", "", "", "", "")
    
    def test_and_send_masterthesis_year_invalid_negative(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "-1", "", "", "", "")
    
    def test_and_send_masterthesis_year_invalid_big(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.send_masterthesis("user8", "Author4", "Title4", "School4", "4444", "", "", "", "")
...
#------------------------------------------------------------
    ...
    def test_delete_masterthesis(self):
        with app.app_context():
            methods.send_masterthesis("user9", "Author9", "Title9", "School9", "1999", "", "", "", "")
...
#------------------------------------------------------------
    ...
    def test_edit_masterthesis(self):
        with app.app_context():
            methods.send_masterthesis("user4", "Author4", "Tämäpoistetaan_title", "School4", "2022", "", "", "", "")
            methods.edit_masterthesis("user4", "Au22", "Author4", "Title_after", "School4", "2022", "2222", "", "", "")
            after = methods.get_references()
            self.assertNotIn("Tämäpoistetaan_title", after[0])
            self.assertIn("2222", after[0])
...
#------------------------------------------------------------
    ...
    def test_get_references_search_reftype(self):
        with app.app_context():
            methods.send_book("user4", "Author4", "Title4", "2022", "Publisher6", "", "", "", "", "", "")
            methods.send_masterthesis("user3", "Author3", "title3", "School3", "2022", "", "", "", "")
            references = methods.get_reference_search("book")
            self.assertIn(references[0][1], "book")
            self.assertNotIn(references[0][1], "masterthesis")
    ...
    def test_get_references_search_key(self):
        with app.app_context():
            methods.send_book("user4", "Author4", "Title4", "2022", "Publisher6", "", "", "", "", "", "")
            methods.send_masterthesis("user3", "Author3", "title3", "School3", "2021", "", "", "", "")
...
#------------------------------------------------------------
    ...
    def test_and_edit_masterthesis_too_long_user(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis(self.too_long, "key8", "Author4", "Title4", "School4", "2022", "", "", "", "")
    
    def test_and_edit_masterthesis_too_long_author(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", self.too_long, "Title4", "School4", "2022", "", "", "", "")
    
    def test_and_edit_masterthesis_too_long_title(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", self.too_long, "School4", "2022", "", "", "", "")
    
    def test_and_edit_masterthesis_too_long_school(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", self.too_long, "2022", "", "", "", "")
    
    def test_and_edit_masterthesis_too_long_type(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "2022", self.too_long, "", "", "")

    def test_and_edit_masterthesis_too_long_address(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "2022", "", self.too_long, "", "")

    def test_and_edit_masterthesis_too_long_note(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "2022", "", "", "", self.too_long)

    def test_and_edit_masterthesis_too_long_month(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "2022", "", "", "väärä", "")
    
    def test_and_edit_masterthesis_year_invalid_string(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "väärä", "", "", "", "")
    
    def test_and_edit_masterthesis_year_invalid_negative(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "-1", "", "", "", "")
    
    def test_and_edit_masterthesis_year_invalid_big(self):
        with app.app_context():
            with self.assertRaises(InvalidInputError):
                methods.edit_masterthesis("user8", "key8", "Author4", "Title4", "School4", "4444", "", "", "", "")
